...
export default function ProductActions({ moq, stock }: ProductActionsProps) {
  const [quantity, setQuantity] = useState<number>(moq);

  const isOutOfStock = stock === 0;
  const isBelowMOQ = quantity < moq;
  const isAboveStock = quantity > stock;
  const isValid = !isOutOfStock && !isBelowMOQ && !isAboveStock;

  const handleIncrement = () => setQuantity(q => q + 100);
  const handleDecrement = () => setQuantity(q => Math.max(0, q - 100));

  const handleAddToCart = () => {
    if (isValid) alert(`Added ${quantity} units to business cart.`);
  };

  const handlePlaceOrder = () => {
     if (isValid) alert(`Proceeding to checkout with ${quantity} units.`);
  };
      ...
      <div className="flex flex-col gap-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
             ...
             {isOutOfStock && <span className="text-red-500 text-sm font-bold flex items-center gap-1.5"><AlertCircle className="w-4 h-4"/> Out of Stock</span>}
          </div>
          ...
          <div className="mt-2 h-5">
             {!isOutOfStock && isBelowMOQ && (
                <span className="text-amber-600 text-xs font-semibold flex items-center gap-1">
                  <AlertCircle className="w-3.5 h-3.5" /> Minimum Order Quantity is {moq} units.
                </span>
             )}
             {!isOutOfStock && isAboveStock && (
                <span className="text-amber-600 text-xs font-semibold flex items-center gap-1">
                  <AlertCircle className="w-3.5 h-3.5" /> Requested amount exceeds current inventory ({stock}).
                </span>
             )}
          </div>
        </div>
